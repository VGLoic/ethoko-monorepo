...
import { createTestLocalStorageProvider } from "@test/helpers/local-storage-provider-factory";
import { TEST_CONSTANTS } from "@test/helpers/test-constants";
import { createTestProjectName } from "@test/helpers/test-utils";
import type { LocalStorageProvider } from "@/storage-provider/local-storage-provider";
import type { LocalStorage } from "@/local-storage";

describe("Push-Pull E2E Tests (Local Storage)", () => {
  let storageProvider: LocalStorageProvider;
  let localStorage: LocalStorage;
  let localStorageCleanup: (() => Promise<void>) | null = null;
  let remoteStorageCleanup: (() => Promise<void>) | null = null;

  beforeEach(async () => {
    const providerSetup = await createTestLocalStorageProvider();
    storageProvider = providerSetup.provider;
    remoteStorageCleanup = providerSetup.cleanup;
  ...
  afterEach(async () => {
    ...
    if (remoteStorageCleanup) {
      await remoteStorageCleanup();
      remoteStorageCleanup = null;
    }
  });
  ...
  test("stores original content files", async () => {
    const project = createTestProjectName(TEST_CONSTANTS.PROJECTS.DEFAULT);
    const artifactPath =
      TEST_CONSTANTS.PATHS.SAMPLE_ARTIFACT.HARDHAT_V2_COUNTER;
