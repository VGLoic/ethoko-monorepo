...
export async function generateArtifactsSummariesAndTypings(
  ...
  const projectsResult = await toAsyncResult(localStorage.listProjects(), opts);
  ...
  if (projects.length === 0) {
    ...
    const emptySummariesResult = await toAsyncResult(
      writeEmptySummaries(localStorage.rootPath, sokoTypingsPath, opts),
      opts,
    );
  ...
  for (const project in summary) {
    ...
    if (!projectSummary) {
      throw new Error(`Unexpected missing summary for project "${project}"`);
    }
  ...
  const writeJsonResult = await toAsyncResult(
    fs.writeFile(
      ...
      JSON.stringify(
        {
          sokoPath: localStorage.rootPath,
          projects: summary,
        },
        null,
        4,
