...
Ethoko CLI will look for a `ethoko.config.json` file in the current working directory, and if not found, it will look for parent directories until it finds one or reaches the root directory. The configuration file can also be specified explicitly using the `--config` flag when running Ethoko CLI commands.

In the configuration file, multiple projects can be defined, each with its own storage configuration. Project only interested in pushing artifacts will likely have a single project defined, while organizations that want to pull artifacts for multiple projects may find it useful to define multiple projects in the configuration file.
...
## Additional artifacts configuration
