...
This section outlines typical developer workflows and how the hybrid global/local config model supports them.

### Journey 1: Solo Developer Starting Fresh

**Context**: Developer starting a new Solidity project, wants simple setup.

**Steps:**

```bash
# 1. Initialize project with local config (current behavior)
cd my-new-project/
ethoko init
...
# → Project name: my-contracts
# → Storage: Filesystem
# → Storage path: .ethoko-storage
# → Compilation output: ./out
# → Pulled artifacts: .ethoko
# → Typings: .ethoko-typings

# 2. Use the project
ethoko push my-contracts:v1.0 --artifact-path ./out
ethoko pull my-contracts:v1.0
```

**Outcome**: Works exactly as before, no global config needed.

---

### Journey 2: Developer Joining a Team with Shared Projects

**Context**: New developer joins team, needs access to company's shared artifact projects.

**Setup (One-Time):**

**Option A: Manual Config (Recommended)**

```bash
# Team provides config snippet
mkdir -p ~/.ethoko
cat > ~/.ethoko/config.json << 'EOF'
{
  ...
  "projects": [
    ...
    {
      "name": "company-periphery-contracts",
      "storage": {
        "type": "aws",
        "awsRegion": "us-east-1",
        "awsBucketName": "company-ethoko-prod",
...
# Configure AWS SSO (standard AWS CLI)
...
# → Use default pulled artifacts path? yes
# → Add project? yes
# → Project name: company-core-contracts
# → Storage: AWS S3
# → Region: us-east-1
# → Bucket: company-ethoko-prod
# → Auth: AWS Profile
# → Profile: company-sso
# → Add another? yes
# → [repeats for company-periphery-contracts]
# → Add another? no
# → Save? yes
```

**Usage (Any Repository):**

```bash
# Works from any directory, no local config needed
cd anywhere/
ethoko pull company-core-contracts:latest

# Create TypeScript types in local repo
cd my-app/
ethoko pull company-core-contracts:latest
ethoko typings
# Now .ethoko-typings/ has types for imported artifacts
```

**Outcome**: Developer can pull shared projects from anywhere, pulled artifacts cached globally.

---

### Journey 3: Developer with Multiple Projects Across Repos

**Context**: Developer working on several projects across different repositories.
...
```json
{
  ...
  "projects": [
    ...
    },
    {
      ...
      "storage": {
        "type": "filesystem",
        "path": "~/.ethoko/storage/side-project"
      }
    }
  ]
}
...
**Repository A** (`repo-a/ethoko.config.json`):
...
**Repository B** (`repo-b/ethoko.config.json`):
...
# In repo A
cd ~/work/repo-a/
ethoko push work-project-a:v2.0    # Uses ./out from local config
ethoko pull work-project-b:latest  # Pulls to global cache

# In repo B
cd ~/work/repo-b/
ethoko push work-project-b:v1.5    # Uses ./artifacts from local config
ethoko pull work-project-a:v2.0    # Already cached globally!

# In side project
cd ~/personal/side-project/
ethoko pull side-project:latest    # Works everywhere
```

**Outcome**:

- Global cache reduces redundant downloads
- Each repo has minimal local config (just `compilationOutputPath`)
- All project definitions in one place (`~/.ethoko/config.json`)

---

### Journey 4: Developer Needs Local Override

**Context**: Developer wants to test changes to project storage locally without affecting global config.
...
```json
{
  "projects": [
    {
      "name": "company-contracts",
      "storage": { "type": "aws" /* production S3 */ }
    }
  ]
}
```

**Local Override** (`./ethoko.config.json`):
...
```bash
# Local config overrides global for "company-contracts"
ethoko push company-contracts:test   # Goes to ./local-test-storage
ethoko pull company-contracts:test   # Pulls from ./local-test-storage

# Other projects still use global config
ethoko pull other-project:latest     # Uses global S3 config
```
...
ethoko config show
# Shows:
# • company-contracts [local - overrides global]
#   Storage: Filesystem (./local-test-storage)
# ⚠ Warning: Local project "company-contracts" overrides global
```

**Outcome**:

- Local testing without touching global config
- Clear visibility into override behavior
- Delete local config to revert to global

---

### Journey 5: Team Onboarding (Standardized Setup)

**Context**: DevOps wants to standardize Ethoko setup across entire team.

**Approach: Documented Config Template**
...
2. Set up global configuration:
   mkdir -p ~/.ethoko
   curl https://internal-wiki.company.com/ethoko-config.json > ~/.ethoko/config.json
...
Done! You can now use Ethoko in any repository.
```

**Config Template** (hosted on internal wiki):
...
```json
{
  ...
  "projects": [
    ...
    {
      "name": "company-test-contracts",
      "storage": {
        "type": "aws",
        "awsRegion": "us-east-1",
        "awsBucketName": "company-ethoko-staging",
        "awsProfile": "company-sso"
      }
    }
  ]
...
- New developers onboarded in minutes
- Consistent configuration across team
- Easy to update (update wiki, team re-downloads)

---

### Journey 6: CI/CD Pipeline

**Context**: CI needs to pull artifacts without interactive configuration.

**Approach: Mix of Global Config and Environment Variables**

**Global Config in CI** (mounted or installed):

```json
{
  "pulledArtifactsPath": "/tmp/ethoko-cache",
  "projects": [
    {
      "name": "company-contracts",
      "storage": {
...
steps:
  - name: Setup Ethoko
    run: |
      mkdir -p ~/.ethoko
      echo '${{ secrets.ETHOKO_CONFIG }}' > ~/.ethoko/config.json
  ...
  - name: Pull artifacts
    run: ethoko pull company-contracts:${{ github.ref_name }}

  - name: Deploy
    run: # ... deployment script using pulled artifacts
```

**Outcome**:

- No repository-specific Ethoko config needed
- Centralized credential management via CI secrets
- Global cache speeds up repeated runs

---

## Command Summary

Based on the developer journeys and minimal command philosophy, here's the final command set:
